...
产物覆盖 popover(popover 各焦点 + 全新安装/错误/正常/偏低/临界/刷新中 + 口径乙两态 + 到期各态与 Kimi 手动标记态,浅色 + 深色)、设置窗口(通用 + 登录开关两态 + 三种凭据形态 + Kimi 套餐状态两态 + 保存成功 + 钥匙串失败横幅)、菜单栏图标全态(恒为 Kimi 一家:绿/黄/红数字 + 陈旧降透明度 + 灰「—」三态,其中 `menubar-icon-kimi-vs-tighter-glm.png` 是「GLM 更紧也不改图标」的验收态)。
...
图标重生成(改设计时):`<swift> scripts/icon/generate.swift`——AppKit 绘制 + `iconutil` 成 icns,
中间 iconset 已 gitignore;仓库提交的 `scripts/icon/AppIcon.icns` 即其产物,`make-app.sh` 负责挂进 bundle。

稳定签名身份(#50):`make-app.sh` 优先用自签证书 `UsageMonitor-dev` 签名——ad-hoc 的 cdhash
随每次重打包漂移,钥匙串 ACL 会把新构建当成新 App,首启读凭据就弹 login keychain 密码框。
证书不存在时回落 ad-hoc 并提示。一次性创建:`scripts/make-signing-cert.sh`(CN 用 ASCII,
openssl 写中文 CN 会乱码);首次读取既有凭据时点「始终允许」一次即永久(或在设置里
重存一次凭据,由稳定身份创建,ACL 直接含它)。

验证通知同屏不打扰(C4,#42)与到期提醒三类(#57;通知需 bundle 身份,须打包后运行):`scripts/make-app.sh debug`,
退出已运行实例后 `open build/用量监视器.app --args --debug-test-notifications 8`——
每 8 秒轮转发一条测试通知(用量临界 / 套餐即将到期 / 套餐已到期 / 套餐已恢复;identifier 带序号,
通知中心可累积):popover 打开或设置窗口前置时无横幅无声音、仅通知中心有记录;两者都不可见时恢复横幅 + 声音。
点击通知直达焦点卡(与真实通知同路由)。
