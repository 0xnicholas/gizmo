...
产物覆盖 popover(popover 各焦点 + 全新安装/错误/正常/偏低/临界/刷新中 + 到期各态与 Kimi 手动标记态,浅色 + 深色)、设置窗口(通用 + 登录开关两态 + 三种凭据形态 + Kimi 套餐状态两态 + 保存成功 + 钥匙串失败横幅)、菜单栏图标全态(绿/黄/红数字 + 灰「—」 + DeepSeek-only 彩色「—」与 DeepSeek 临界×窗口 65% 的口径乙形态 + 到期全态)。
